...
impl<'c, 'w, 's, 'a, 'world> ComponentCommands<'c, 'w, 's, 'a, 'world> {
        ...
        L::Object: Clone + Component,
    {
        let Some(current_value) = self.get::<L::Object>().and_then(L::try_get_value) else {
            self.insert(
                Transition::<L>::new(TransitionStep::new_arc(default_initial_value.clone(), None, NextStep::None)));
        ...
        self.insert(Transition::<L>::new(TransitionStep::new_arc(destination, speed, NextStep::None)) );
